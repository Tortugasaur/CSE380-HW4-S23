...
export default class HealerBehavior extends NPCBehavior {
  ...
  public initializeAI(owner: NPCActor, opts: Record<string, any>): void {
    ...
    let pickupHP = new PickupItem(this, this.owner);
    pickupHP.targets = scene.getHealthpacks();
    pickupHP.targetFinder = new BasicFinder(
      ClosestPositioned(this.owner),
      ItemFilter(Healthpack),
      VisibleItemFilter()
    );
    pickupHP.addPrecondition(HealerStatuses.HPACK_EXISTS);
    pickupHP.addEffect(HealerStatuses.HAS_HPACK);
    pickupHP.cost = 1;
    this.addState(HealerActions.PICKUP_HPACK, pickupHP);

    let useHP = new UseHealthpack(this, this.owner);
    useHP.targets = scene.getBattlers();
    useHP.targetFinder = new BasicFinder(
      ClosestPositioned(this.owner),
      BattlerActiveFilter(),
